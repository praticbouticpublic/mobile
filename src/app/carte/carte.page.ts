...
export class CartePage implements OnInit {
    ...
    constructor(private httpClient: HttpClient, private alert: AlertController, private router: Router, private route: ActivatedRoute,
        private order: OrderService, private session: SessionfrontService, private initsession: InitSession, public menuCtrl: MenuController,
        ...
        this.httpClient.post<Categorie[]>(environment.apiroot + 'front', objcat, await this.header.buildHttpOptions()).pipe(
            ...
            next: (categories: Categorie[]) => {
                ...
                if (categories.length > 0)
                    this.getArticles(bouticid, this.categories[this.nocat]);
            },
            error: (err: any) => {
        ...
        this.httpClient.post<Article[]>(environment.apiroot + 'front', objart, await this.header.buildHttpOptions()).pipe(
            ...
            next: (articles: Article[]) => {
                ...
                if (articles.length > 0)
                    this.getImages(bouticid, this.articles[this.nocat][this.noart[this.nocat]]);
    ...
    async getImages( bouticid: number, article: Article) {
        ...
        this.httpClient.post<Image[]>(environment.apiroot + 'front', objimg, await this.header.buildHttpOptions()).pipe(
            ...
            next: (images: Image[]) => {
                ...
                this.nogrp = 0;
                this.getGroupes( bouticid, this.articles[this.nocat][this.noart[this.nocat]]);
            },
            error: (err: any) => {
                this.presentErrAlert(strings.ErrConnect, err.error.error);
            }
    ...
    async getGroupes( bouticid: number, article: Article) {
        ...
        this.httpClient.post<Groupe[]>(environment.apiroot + 'front', objgrp, await this.header.buildHttpOptions()).pipe(
            ...
            next: (groupes: Groupe[]) => {
                ...
                this.totaliser();
                if (groupes.length > 0)
                    this.getOptions( bouticid, groupes[this.nogrp]);
    ...
    async getOptions(bouticid: number, grp: Groupe) {
        ...
        this.httpClient.post<Option[]>(environment.apiroot + 'front', objopt, await this.header.buildHttpOptions()).pipe(
            ...
            next: (options: Option[]) => {
                ...
                if (this.articles[this.nocat][this.noart[this.nocat]].qte) {
                    for (let cur = 1; cur <= this.articles[this.nocat][this.noart[this.nocat]].qte; cur++) {
                        if (grp.options.length > 0) {
                            this.getStoredGrpSel(this.articles[this.nocat][this.noart[this.nocat]].id, grp.id, cur).then(optsel => {
                                if (grp.multi)
                ...
                if (this.nogrp < this.articles[this.nocat][this.noart[this.nocat]].groupes.length)
                    this.getOptions(bouticid, this.articles[this.nocat][this.noart[this.nocat]].groupes[this.nogrp])
                else if (this.noart[this.nocat] + 1 < this.articles[this.nocat].length) {
                    this.noart[this.nocat]++;
                    this.getImages(bouticid, this.articles[this.nocat][this.noart[this.nocat]]);
                }
    ...
    Poursuivre() {
        if (this.somme >= this.mntcmdmini) {
            ...
            this.router.navigate(['admin/getinfo'])
        }
        else {
                        ...
                        buttons: [
                            {
                                text: 'OK',
                                handler: data => {
                                    this.router.navigate(['admin/debut/' + aliasboutic + '/' + method + '/' + table])
                                }
                            }
                        ],
                    });
