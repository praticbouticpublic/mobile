...
export class OrderlinesPage implements OnInit {
    ...
    constructor(public router: Router, public menuCtrl: MenuController, public getback: GetbackService, public httpClient: HttpClient,
        ...
        public model: ModeleService, public global: GlobalService, public header:HeaderService ) {
    ...
    ngOnInit() {
        ...
        const id = +this.router.url.substring(this.router.url.lastIndexOf('/') + 1);
        const str = this.router.url.substring(0, this.router.url.lastIndexOf('/'));
        this.idcmd = +str.substring(str.lastIndexOf('/') + 1);
        const table = 'lignecmd';
    ...
    goBack() {
        ...
        this.router.navigate(['admin/detailsorder/' + this.idcmd], { replaceUrl: true });
    }
