...
export class  SubmitBase {
    ...
    constructor(public model: ModeleService, public storage: StorageService, public getback:GetbackService,
    ...
    async onSubmitAction(event: Event, act: string)  {
        this.isSubmitted = true;
            ...
            if (error === false) {
                if (act === 'update') {
                    this.model.updaterow(this.bouticid, this.model.getTable(this.numtable).nom, this.row, pknom, this.input_id, this).then( async () => {
                        await this.appendImgtoBd().then(() => this.goBack());
                    });
                }
                else if (act === 'insert') {
                    (await this.model.insertrow(this.bouticid, this.model.getTable(this.numtable).nom, this.row).then()).subscribe({
                        next: (response: any) => {
                            this.input_id = +response.id;
                            this.appendImgtoBd();
                            this.goBack();
                        }, error: (err: any) => {
                            this.presentAlert(err.error.error);
