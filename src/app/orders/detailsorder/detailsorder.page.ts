...
export class DetailsorderPage implements OnInit {
    ...
    constructor(public router: Router, public menuCtrl: MenuController, public getback: GetbackService, public httpClient: HttpClient,
        public alertController: AlertController, public platform: Platform, public storage: StorageService, public model: ModeleService,
        public global: GlobalService, public header:HeaderService ) {
        this.global.setFormComponent(this);
        this.latable = this.model.getTableParNom(this.tablestr);
    ...
    ionViewDidEnter()
    {
        ...
        this.numtable = this.model.getnumtable(table);
            this.storage.get('bouticid').then(bouticid => {
                this.bouticid = bouticid;
                this.detail(this.bouticid, this.numtable, this.id, 5, 0);
            });
    }

    goToOrderLinesPage(idlncmd: any) {
        this.router.navigate(['admin/orderlines/' + this.id + '/' + idlncmd], { replaceUrl: false });
    }
    ...
    onChangeNbRes(event: Event) {
        ...
        this.changeFunc(this.bouticid, this.vue, this.place, this.tablestr, elem.value, this.selcol, this.selid);
        localStorage.setItem('praticboutic_ctrl_' + this.global.getServer() + '_' + this.global.getLogin() + '_rppid' + this.model.getnumtable(this.tablestr), elem.value);
        if (this.selid == 0)
            localStorage.setItem('praticboutic_ctrl_' + this.global.getServer() + '_' + this.global.getLogin() + '_nav' + this.model.getnumtable(this.tablestr), myGlobals.defoffset.toString());
    }
    ...
    onPrevLinkClick(event: Event) {
        ...
        if ((this.offset - this.limite) >= 0) {
            this.gettable(this.bouticid, this.vue, this.place, this.tablestr, this.limite, ((this.activepage - 1) * this.limite), this.selcol, this.selid);
            if (this.selid == 0)
                localStorage.setItem('praticboutic_ctrl_' + this.global.getServer() + '_' + this.global.getLogin() + '_nav' + this.model.getnumtable(this.tablestr), (this.offset - this.limite).toString());
        }
    }
    ...
    onPageLinkClick(event: Event, k: any) {
        if (k !== this.activepage) {
            this.gettable(this.bouticid, this.vue, this.place, this.tablestr, this.limite, ((k - 1) * this.limite), this.selcol, this.selid);
            if (this.selid == 0)
                localStorage.setItem('praticboutic_ctrl_' + this.global.getServer() + '_' + this.global.getLogin() + '_nav' + this.model.getnumtable(this.tablestr), ((k - 1) * this.limite).toString());
        }
    }
